...
fn benchmark_trigger_key_match() {
    ...
    println!(
        "Trigger key match: {} iterations in {:?}",
        iterations, elapsed
    );
    ...
    assert!(
        avg_ns < 1000.0,
        "Trigger matching too slow: {:.2} ns",
        avg_ns
    );
}
...
fn benchmark_context_match() {
    ...
    assert!(
        avg_ns < 5000.0,
        "Context matching too slow: {:.2} ns",
        avg_ns
    );
}
...
fn benchmark_layer_stack_operations() {
    ...
    println!(
        "Layer stack operations: {} iterations in {:?}",
        iterations, elapsed
    );
    ...
    assert!(
        avg_ns < 2000.0,
        "Layer stack operations too slow: {:.2} ns",
        avg_ns
    );
}
...
fn benchmark_mapping_rule_match() {
    ...
    println!(
        "Mapping rule match: {} iterations in {:?}",
        iterations, elapsed
    );
    println!("Average: {:.2} ns per match", avg_ns);
...
fn benchmark_action_creation() {
    ...
    for _ in 0..iterations {
        ...
        let _ = Action::mouse(MouseAction::Move {
            x: 100,
            y: 100,
            relative: true,
        });
...
fn benchmark_window_calculations() {
    ...
    println!(
        "Window calculations: {} iterations in {:?}",
        iterations, elapsed
    );
    ...
    assert!(
        avg_ns < 100.0,
        "Window calculations too slow: {:.2} ns",
        avg_ns
    );
}
...
fn benchmark_real_world_scenario() {
    ...
    for i in 0..5 {
        let mut layer =
            Layer::new(&format!("layer_{}", i), 0x3A + i as u16, 0x14 + i as u16);
        for j in 0..10 {
            layer.add_mapping(
    ...
    println!(
        "Real world scenario: {} iterations in {:?}",
        iterations, elapsed
    );
    ...
    assert!(
        avg_us < 50.0,
        "Real world scenario too slow: {:.2} μs",
        avg_us
    );
}
