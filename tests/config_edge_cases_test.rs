...
"#;
    ...
    let result = Config::from_str(config_str);
    assert!(result.is_ok(), "Failed to parse full config: {:?}", result.err());
...
"#;
    ...
    assert!(result_hold.is_ok(), "Hold mode failed: {:?}", result_hold.err());
    assert!(result_toggle.is_ok(), "Toggle mode failed: {:?}", result_toggle.err());
}
...
"#;
    ...
    let result = Config::from_str(config_str);
    assert!(result.is_ok(), "Failed to parse window actions: {:?}", result.err());
...
"#;
    ...
    let result = Config::from_str(config_str);
    assert!(result.is_ok(), "Failed to parse complex key mappings: {:?}", result.err());
    
    let config = result.unwrap();
    assert_eq!(config.keyboard.remap.get("CapsLock").unwrap(), "Ctrl+Alt+Win");
}
...
"#;
    ...
    let result = Config::from_str(config_str);
    assert!(result.is_ok(), "Failed to parse nested layers: {:?}", result.err());
